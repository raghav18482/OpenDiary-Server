...
const multer = require("multer");
 ...
 const path = require("path"); //we are using req.path so that we will get path url
...
const upload = multer({ 
  ...
  //fileFilter: fileFilter,// according to file filter it will store the image
});

//adding and update profile image
router
  .route("/add/image")
  //using patch we will update the data 
  ...
  Profile.findOne({ username: req.decoded.username }, (err, result) => {
    if (err) return res.json({ err: err });
      ...
      return res.json({ status: true,
          username: req.decoded.username//here we are sending user name along with true/false
         });
    }
  });
});
